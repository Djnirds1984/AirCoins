...
if [ "$IS_ARM" = true ]; then
    ...
    echo ""
    echo -e "  ${YELLOW}No WiFi yet?${NC} The admin panel is also reachable via the"
    echo -e "    Ethernet IP from any device on the same LAN:"
    echo -e "    ${CYAN}http://$(hostname -I 2>/dev/null | awk '{print $1}')/admin.html${NC}"
    echo ""
