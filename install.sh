...
if [ -f "$SYSTEM_DIR/usr/local/bin/aircoins-api/$PREBUILT" ] && \
   file "$SYSTEM_DIR/usr/local/bin/aircoins-api/$PREBUILT" 2>/dev/null | grep -q "ELF"; then
    echo "  ✓ Pre-compiled binary detected, skipping Go build"
    mkdir -p /usr/local/bin/aircoins-api
    cp "$SYSTEM_DIR/usr/local/bin/aircoins-api/$PREBUILT" /usr/local/bin/aircoins-api/aircoins-api
